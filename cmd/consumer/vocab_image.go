package consumer

import (
	"context"
	"encoding/json"
	"fmt"
	"path/filepath"

	"github.com/hibiken/asynq"
	"github.com/npvu1510/crawl-en-vocab/internal"
	"github.com/npvu1510/crawl-en-vocab/internal/model"
	"github.com/npvu1510/crawl-en-vocab/internal/service"
	"github.com/npvu1510/crawl-en-vocab/pkg/config"
	"github.com/npvu1510/crawl-en-vocab/pkg/gcs"
	"github.com/npvu1510/crawl-en-vocab/pkg/utils"
	"github.com/spf13/cobra"
)

var VocabImageConsumerCmd = &cobra.Command{
	Use:   "vocab-image-consumer",
	Short: "Consume vocabulary image tasks",
	Run: func(cmd *cobra.Command, args []string) {
		internal.Invoke(consumerCmd).Start(context.Background())
	},
}

type VocabImageConsumer struct {
	Service service.IDictionaryService
	Config  *config.Config
}

func (v *VocabImageConsumer) handlerVocabImageTask(ctx context.Context, t *asynq.Task) error {

	// UNMARSHAL
	var d *model.Dictionary
	if err := json.Unmarshal(t.Payload(), &d); err != nil {
		return fmt.Errorf("HandleAudioTask Unmarshal failed: %v", err)
	}

	// HANDLE TASK (STORAGE IMAGE)
	gcsService := gcs.Service

	filename := utils.Definition2FileName(d.Definition) + ".svg"
	fullFilePath := filepath.Join(v.Config.App.LOCAL_IMAGE_PATH, filename)

	imagePath, err := utils.DownloadImage(d.Image, "./"+fullFilePath)
	utils.CheckError(err)

	publicUrl, err := gcsService.UploadFile(imagePath, "/vu1/images/"+filename)
	if err != nil {
		return fmt.Errorf("HandleAudioTask storage image failed: %v", err)
	}

	// Update db with publicUrl
	v.Service.UpdateImage(d, publicUrl)

	return nil
}
